        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-start mb-20">
          ...
          <motion.div className="lg:col-span-5 order-2" initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}>
            <div className="grid grid-cols-2 gap-4">
              {highlights.map((h, i) => (
                <motion.div key={h.label} initial={{ opacity: 0, y: 24, scale: 0.9 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, margin: "-40px" }} transition={{ delay: 0.3 + i * 0.12, duration: 0.5, type: "spring", stiffness: 120 }} whileHover={{ y: -8, scale: 1.04, transition: { duration: 0.25 } }} className="group relative glass-panel-ios rounded-2xl p-6 overflow-hidden cursor-default">
