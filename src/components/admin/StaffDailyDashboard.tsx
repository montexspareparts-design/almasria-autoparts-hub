      ...
      <Accordion type="single" collapsible value={openSection} onValueChange={handleAccordionChange} className="space-y-3">
        ...
        <AccordionItem value="performance" className="border rounded-xl bg-card overflow-hidden">
          ...
          <AccordionContent className="px-4 pb-4">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <MiniStat
                label="عملاء تم التواصل معهم اليوم"
                value={stats.todayLeadsContacted}
                icon={Phone}
                onClick={() => onNavigate?.("customers")}
              />
              <MiniStat
                label="إجمالي الطلبات المعالجة"
              ...
              />
              <MiniStat
                label="عملاء تم تحويلهم لتجار"
                value={stats.totalLeadsConverted}
                highlight
                icon={UserPlus}
                onClick={() => onNavigate?.("leads")}
              />
