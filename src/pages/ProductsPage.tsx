                  ...
                  {paginatedProducts.map((product, i) => (
                    ...
                    >
                      <div className="aspect-square bg-white relative overflow-hidden">
                        {product.image_url ? <img src={product.image_url} alt={product.name_ar} className="w-full h-full object-contain p-3 group-hover:scale-105 transition-transform duration-500 mix-blend-multiply" loading="lazy" /> : <div className="w-full h-full flex items-center justify-center"><Package className="w-12 h-12 text-muted-foreground/20" /></div>}
                      ...
                      <div className="p-3 sm:p-4" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          {product.stock_quantity > 0 ? <span className="text-[10px] sm:text-[11px] text-green-600 bg-green-50 px-2 py-0.5 rounded font-semibold">متوفر</span> : <span className="text-[10px] sm:text-[11px] text-red-600 bg-red-50 px-2 py-0.5 rounded font-semibold">غير متوفر</span>}
                        </div>
                        ...
                        {!user ? (
                          <Button variant="outline" size="sm" className="w-full mt-1 gap-2 text-xs" onClick={() => { toast({ title: "يجب تسجيل الدخول أولاً" }); navigate("/auth"); }}><Lock className="w-3.5 h-3.5" />سجل دخولك لعرض السعر</Button>
                        ) : !isDealer ? (
                          <><div className="text-primary font-black text-base sm:text-lg">{product.base_price.toLocaleString("ar-EG")} ج.م</div><p className="text-[10px] sm:text-[11px] text-muted-foreground">سعر قطاعي</p></>
                        ) : viewedProductIds.includes(product.id) ? (
                          <><div className="text-primary font-black text-base sm:text-lg">{getProductPrice(product).toLocaleString("ar-EG")} ج.م</div><p className="text-[10px] sm:text-[11px] text-green-600 font-semibold">سعر الجملة الخاص بك</p></>
                        ) : !limitReached ? (
                          <Button variant="outline" size="sm" className="w-full mt-1 gap-2 text-xs" onClick={() => recordView(product.id)}><Eye className="w-3.5 h-3.5" />اعرض السعر ({DAILY_LIMIT - dailyViewCount} متبقي)</Button>
                        ) : (
                        ...
                        {product.stock_quantity > 0 && user && (!isDealer || viewedProductIds.includes(product.id)) && (
                          <Button size="sm" className="w-full mt-3 gap-2 text-xs" onClick={() => handleAddToCart(product)}><ShoppingCart className="w-3.5 h-3.5" />أضف للسلة</Button>
                        )}
                      </div>
                    </motion.div>
                  ))}
                  ...
                  {paginatedProducts.map((product, i) => (
                    ...
                    >
                      <div className="w-28 sm:w-36 shrink-0 bg-white flex items-center justify-center p-3">
                        {product.image_url ? <img src={product.image_url} alt={product.name_ar} className="w-full h-full object-contain mix-blend-multiply" loading="lazy" /> : <Package className="w-10 h-10 text-muted-foreground/20" />}
                      </div>
