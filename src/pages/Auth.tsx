...
import { Eye, EyeOff, Phone, Mail, User, MapPin, ShoppingBag, ArrowLeft } from "lucide-react";
    ...
    <div className="min-h-screen bg-muted/30 flex items-center justify-center px-4 py-8" dir="rtl">
      <div className="w-full max-w-[420px]">
        ...
        <div className="text-center mb-6">
          ...
          <div className="inline-flex items-center gap-2 bg-blue-50 text-blue-700 text-[11px] font-bold px-3 py-1.5 rounded-full mb-4">
        ...
        <div className="bg-card border border-border/50 rounded-xl p-5 shadow-sm">
          ...
          {!forgotMode && (
            <>
              ...
              <div className="flex bg-muted/50 rounded-lg p-0.5 mb-4">
                <button onClick={() => setIsLogin(true)} className={`flex-1 text-xs font-bold py-2 rounded-md transition-all ${isLogin ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}>تسجيل الدخول</button>
                <button onClick={() => setIsLogin(false)} className={`flex-1 text-xs font-bold py-2 rounded-md transition-all ${!isLogin ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'}`}>حساب جديد</button>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {!isLogin && (
                <div className="space-y-1.5">
                  <Label className="text-xs text-foreground">الاسم الكامل <span className="text-primary">*</span></Label>
              ...
              {!isLogin && (
                <div className="space-y-1.5">
                  <Label className="text-xs text-foreground">العنوان <span className="text-muted-foreground text-[10px]">(اختياري)</span></Label>
                  <div className="relative">
        ...
        {!forgotMode && (
          <div className="mt-4 space-y-3">
            <div className="bg-card border border-border/50 rounded-xl p-4 text-center shadow-sm">
              <p className="text-[11px] text-muted-foreground mb-2">هل أنت تاجر أو موزع معتمد؟</p>
              <Button variant="outline" size="sm" className="gap-2 text-xs font-bold h-8 border-primary/30 text-primary hover:bg-primary/5" asChild>
