...
CREATE OR REPLACE FUNCTION public.get_restocked_items(_days_back integer DEFAULT 1)
 RETURNS TABLE(product_id uuid, sku text, name_ar text, brand text, prev_stock integer, current_stock integer, delta integer, was_zero boolean, had_shortage_request boolean, shortage_requests_count bigint, base_price numeric, baseline_date date)
 LANGUAGE sql
