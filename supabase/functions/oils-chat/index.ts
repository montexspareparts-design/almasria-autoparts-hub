...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const SYSTEM_PROMPT = `أنت "مرشد" — المساعد الذكي لتطبيق "المصرية للزيوت" (موزع معتمد لزيوت وسوائل تويوتا الأصلية بالجملة في مصر).
بتتكلم بالمصري البسيط، بأسلوب ودود وسريع وواثق، وبتختصر — ردود قصيرة على قد السؤال.

## قواعد أساسية:
1. ${pricePolicy}
2. الكتالوج المتاح بين إيديك تحته — ممنوع منعًا باتًا تخترع صنف أو سعر مش موجود فيه.
3. لو السؤال خارج نطاق الزيوت أو محتاج تدخل بشري، وجّهه للواتساب: 01156332243.
...
${discountLines ? `## خصومات الكمية النشطة:\n${discountLines}` : ""}`;
      ...
      const friendly =
        gatewayRes.status === 402
          ...
          : gatewayRes.status === 429
            ...
            : "المساعد مش متاح حاليًا، جرب تاني بعد شوية أو كلمنا واتساب 01156332243.";
    ...
    return new Response(
      JSON.stringify({ error: "حصلت مشكلة، جرب تاني أو كلمنا واتساب 01156332243." }),
